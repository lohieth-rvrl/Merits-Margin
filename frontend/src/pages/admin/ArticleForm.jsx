...
export default function ArticleForm() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((a) => {
        ...
        setLastScheduledFor(a.scheduledFor || null);
  ...
  function buildPayload(status) {
    return {
      ...
      status,
      scheduledFor: status === "scheduled" ? form.scheduledFor : null,
    };
  }
  ...
  async function handleSave(status) {
    ...
    if (status === "scheduled" && new Date(form.scheduledFor) <= new Date()) {
      setError("Scheduled time needs to be in the future.");
      return;
    }
    setError(null);
    setSaving(status);
      ...
      <div className="lr-editor-topbar py-2 sticky-top">
        <div className="container d-flex align-items-center justify-content-between">
          <div className="d-flex align-items-center gap-3">
            ...
            <span className="lr-status-pill">{statusLabel(lastStatus, lastScheduledFor)}</span>
          </div>
          <div className="d-flex align-items-center gap-3">
      ...
      {showPublishModal && (
        <div className="lr-modal-backdrop" onClick={() => setShowPublishModal(false)}>
          <div className="lr-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mb-4">
              <label className="form-label fw-semibold">
                Schedule for later <span className="text-secondary fw-normal">(optional — leave blank to publish immediately)</span>
              </label>
              <input
                type="datetime-local"
                className="form-control"
            ...
            <div className="d-flex justify-content-end gap-2">
              ...
              <button
                ...
                disabled={saving !== null}
                onClick={async () => {
                  const targetStatus = form.scheduledFor ? "scheduled" : "published";
              ...
              >
                {saving
                  ? "Saving…"
                  : form.scheduledFor
                  ? `Schedule for ${new Date(form.scheduledFor).toLocaleDateString("en-US", { month: "short", day: "numeric" })}`
                  : "Publish now"}
              </button>
            </div>
          </div>
